...
if (isset($_POST['btnChercher'])) {
	...
	// sont bien présentes
	if (! parametresControle('post', ['btnChercher',
									'radNom', 'txtNom'])){
...
if (count($_SESSION['recherche']) == 2){
	...
	if (mysqli_num_rows($r) == 0){
		...
		htmlEnteteTable(['Nom', 'Prénom', 'Pays'], 'tab-bd');

		while ($enr = mysqli_fetch_assoc($r)) {
			$enr = tabProtegerSorties($enr);

			$id = chiffreSigneURL($enr['auID']);
...
echo '<form method="POST" class="maj" ',
		...
		'<p class="pagination"> Veuillez saisir un critère '.
		'de recherche non vide</p>': '',
		'<p class="pagination">',
