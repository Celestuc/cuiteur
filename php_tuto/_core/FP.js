	...
	*/
	...
	makeBasPage: function() {
		...
		if (B !== null) {
			H = this.pageAfter	+
				...
				this.pageBefore +
				'&copy; <a href="mailto:' + this.mailTo + '">Eric Merlet</a>';
			B.innerHTML = H;
		}
	},
	...
	initCodeMirror: function() {
		...
		for (i = 0; i < TCode.length; i++) {
			...
			this.TCodeSource[i] = TxtArea.innerHTML.replace(r1, '<').replace(r2, '>').replace(r4, '&');
...
FP.Search =  {
	...
	getHTMLBoite: function() {
		return '<form onsubmit="top.FP.Search.makeResultats();return false;" id="SRCH-bloc" style="display: none; margin: 0">' +
				'<div id="SRCH-btnClose" onclick="top.FP.Search.showBoite()"></div>' +
				'<label for="SRCH-texte">Rechercher</label> ' +
				'<input type="text" name="SRCH-texte" id="SRCH-texte" list="SRCH-datalist" ' +
					'title="Saisir une sous-chaine constituée de lettres et d\'underscores avec éventuellement des tirets au milieu. Rq : vous pouvez par exemple utiliser &apos;_post&apos; pour &quot;$_POST&quot;." ' +
	...
	inputCaractere: function() {
		var mot = this.oInput.value.trim().toLowerCase();
		...
		mot = mot.replace(this.ERy, 'y');
		mot = mot.replace(/[^a-z_-]/g, '');
		mot = mot.replace(/^-/g, '');
		// mot = mot.replace(/-$/g, ''); // plus de possibilité de saisir le - !
